...
import { addPermission } from "@/actions/permissions";
...
export function PermissionAddForm({ guildId, roles }: { guildId: string; roles: Option[] }) {
  const [state, formAction, pending] = useActionState<ActionResult | null, FormData>(addPermission, null);
  ...
  return (
    <form action={formAction} className="grid items-end gap-3 rounded-lg border border-border/60 p-4 sm:grid-cols-[1fr_1fr_auto]">
      <input type="hidden" name="guildId" value={guildId} />
      <div className="space-y-1.5">
        <Label htmlFor="command">Command</Label>
        <select id="command" name="command" className={controlClass} defaultValue="">
          <option value="" disabled>
            Pick a command…
          </option>
          {RESTRICTED_COMMANDS.map((c) => (
